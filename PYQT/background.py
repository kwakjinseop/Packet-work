...
#
#         self.table1.setColumnCount(n_cols)
#
...
#
#         self.table1.setItem(0, 1, QTableWidgetItem("STX\n=======================\n" + (hex(packet[0]))))  # STX
#         STX_O = str(hex(packet[0]))
#         self.table1.setItem(0, 2, QTableWidgetItem(
#             "Time\n=======================\n" + str.join("", ("0x%02X " % i for i in packet[1:5]))))  # Time
#         self.table1.setItem(0, 3, QTableWidgetItem(
#             "Checksum\n=======================\n" + str.join("", ("0x%02X " % i for i in packet[5:9]))))  # Checksum
#         CheckSum_O = str.join("", ("0x%02X " % i for i in packet[5:9]))
#         self.table1.setItem(0, 4, QTableWidgetItem(
#             "Reserved\n=======================\n" + str.join("", ("0x%02X " % i for i in packet[9:13]))))  # Reserved
#         i = 14
#         while (i < len(packet)):
#             Length_1 += 1
#             i += 1
#         self.table1.setItem(0, 5, QTableWidgetItem("Length\n=======================\n" + str(Length_0)))  # Reserved
#         self.table1.setItem(0, 6, QTableWidgetItem(str.join("", ("0x%02X " % i for i in packet[15:]))))  # Payload
#         Payload_O = sum(packet[15:])
#
#         # Length_1 = str.join("", ("0x%02X " % i for i in packet[15:]))
#
#         self.table1.setItem(0, 5, QTableWidgetItem(
#             "Payload\n=======================\n" + str.join("", ("0x%02X " % i for i in packet[14:]))))  # Payload
#
#         #데이터 삽입 시작되는 부분
#         self.table1.setItem(value1, 0, QTableWidgetItem(str(Number)))
#         self.table1.setItem(value1, 1, QTableWidgetItem(textSTX))  # STX
#         self.table1.setItem(value1, 2, QTableWidgetItem(textTime))  # Time
#         self.table1.setItem(value1, 3, QTableWidgetItem(textCheckSum))  # Checksum
#         self.table1.setItem(value1, 4, QTableWidgetItem(textReserved))  # Reserved
#         self.table1.setItem(value1, 5, QTableWidgetItem(str(Length)))  # Length
#         self.table1.setItem(value1, 6, QTableWidgetItem(textPayload))  # Payload
#
#
#         delegate = AlignDelegate(self.table1)
#         self.table1.setItemDelegateForColumn(0, delegate)
#         self.table1.setItemDelegateForColumn(1, delegate)
#         self.table1.setItemDelegateForColumn(2, delegate)
#         self.table1.setItemDelegateForColumn(3, delegate)
#         self.table1.setItemDelegateForColumn(4, delegate)
#         self.table1.setItemDelegateForColumn(5, delegate)
#
...
#         items = self.table1.findItems(s, Qt.MatchContains)
#         if items:
#             item = items[0]
#             self.table1.setCurrentItem(item)
#     def changecolor(self):
#         for x in range(0,Count+1):
#             self.table1.item(x,0).setBackground(QBrush(Qt.darkGreen)) #아이템이 있어야지만 색깔이 칠해짐
#             self.table1.item(x, 1).setBackground(QBrush(Qt.darkYellow))
#             self.table1.item(x, 2).setBackground(QBrush(Qt.gray))
#             self.table1.item(x, 3).setBackground(QBrush(Qt.darkBlue))
#             self.table1.item(x, 4).setBackground(QBrush(Qt.darkRed))
#             self.table1.item(x, 5).setBackground(QBrush(Qt.darkCyan))
#     def __btn4_clicked(self):
#         col_count = self.table1.columnCount()
#         self.table1 .setColumnCount(col_count+1)
#
#     def __btn6_clicked(self):
#         self.table1.removeColumn(1)  # column 삭제
#
#     def __btn7_clicked(self):
#         row_count = self.table1.rowCount()
#         self.table1.setRowCount(row_count + 1)  # row 추가
#
#     def __btn9_clicked(self):
#         self.table1.removeRow(1)  # 1번째 row 삭제
#
